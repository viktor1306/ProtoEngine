...
void Swapchain::recreate() {
    ...
    if (extent.width == 0 || extent.height == 0) {
        return; // Don't recreate if minimized or closing
    }
...
void Swapchain::createSwapchain() {
    ...
    if (imageCount < swapChainSupport.capabilities.minImageCount) imageCount = swapChainSupport.capabilities.minImageCount; // Ensure at least min
    ...
    if (extent.width == 0 || extent.height == 0) {
        extent = {800, 600}; // Fallback or throw? Fallback to avoid crash on init
    }
    ...
    // Fix for validation error: preTransform must vary if rotated? 
    // Usually currentTransform is fine, but let's mask it if needed? No.
