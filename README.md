...
### 6. Воксельний Світ, LOD та Chunk Streaming
- **Greedy Meshing**: Спеціальний алгоритм стиснення геометрії чанків — зливає сусідні однакові грані у великі прямокутники, скорочуючи кількість полігонів у десятки разів.
- **Closed Chunk Meshes**: Кожен чанк ізольовано "закриває" власні межі — між-чанковий culling вимкнено, T-Junction дірки неможливі.
- **LOD (3 рівні)**: Кожен чанк отримує LOD (`0/1/2`) за Евклідовою дистанцією. Гістерезис запобігає миготінню на межах зон.
- **Двозонний Chunk Streaming** (два незалежних параметри у ImGui):
  - **Sphere Radius** — сфера навколо камери: чанки завжди завантажені, LOD оновлюється щокадру.
  - **Camera View Dist** — дальність завантаження по frustum камери (незалежно від сфери).
- **Тришарова Tier-архітектура** (Streaming Out):

  | Tier | Умова | Дія |
  |------|-------|-----|
  | 1 | у сфері | вокселі + GPU меш |
  | 2 | поза сферою, у frustum | вокселі + GPU меш |
  | 3 | поза сферою, поза frustum | вокселі в RAM, GPU меш звільнено |
  | 4 | поза Camera View Dist | вокселі + GPU меш звільнено |

### 7. Інструменти Розробника
- **Dear ImGui**: UI для параметризації рушія (LOD відстані, Sphere Radius, Camera View Dist, статус пам'яті, кількість чанків per LOD).
- **Debug Camera**: режим де основна камера заморожується, а розробник може облетіти сцену іншою — для відладки frustum culling та стрімінгу.
- **Shader Hot-Reloading**: Редагування шейдерів в реальному часі без перезапуску (через `ShaderHotReloader`).
